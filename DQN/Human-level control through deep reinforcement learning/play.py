...
def save_gif(gifs):
    ...
    anim.save("playPong.gif", writer='pillow', fps=30)
...
if __name__ == '__main__':
    ...
    agent = DQNagent(in_channels=state_channel, action_space=action_space, USE_CUDA=USE_CUDA, epsilon=0)
    # 载入保存的神经网络模型
    myQnet = Q_net(in_channels=state_channel, n_actions=action_space.n)
    myQnet.load_state_dict(torch.load('Pong_Qnet3.pt'))
    if USE_CUDA:
        myQnet = myQnet.cuda()
    agent.Qnet = myQnet
    ...
    fig.savefig('play_rewards.png')
    
    print('DQN playing ended successfully!')
    env.close()
