...
class DDQNagent:
    ...
    def compute_td_loss(self, states, actions, rewards, next_states, is_done, gamma=0.99):
        ...
        predicted_next_values = self.Qnet(next_states)
        predicted_next_values_target = self.target_Qnet(next_states)
        
        # torch.max(a, 1):(n,2),第二列为一行最大值元素的位置
        # torch.gather()返回是一行[],用.unsqueeze(1)增加维度,变成[[],[],...]
        next_qvalues = predicted_next_values_target.\
            gather(1, torch.max(predicted_next_values, 1)[1].unsqueeze(1)).squeeze(1)
        
        td_target = rewards + gamma*next_qvalues
