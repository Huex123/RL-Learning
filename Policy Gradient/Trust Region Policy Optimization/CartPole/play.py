...
def save_gif(gifs):
    ...
    anim.save("TRPO_play_CartPole.gif", writer='pillow', fps=30)
...
if __name__ == '__main__':
    ...
    # Playing TRPO in CartPole-v1
    env = gym.make('CartPole-v1', render_mode='rgb_array')
    torch.manual_seed(0)
    agent = TRPO(hidden_dim, env.observation_space, env.action_space, device=device)
    # 加载模型
    agent.actor.load_state_dict(torch.load('CartPole_TRPO_actor_net1.pt'))
    agent.critic.load_state_dict(torch.load('CartPole_TRPO_critic_net1.pt'))
    ...
    axes.set_title("TRPO_play_rewards")
    axes.set_xlabel("episode")
    axes.set_ylabel("episode reward")
    fig.savefig('TRPO_play_rewards.png')

    print('TRPO playing ended successfully!')
    env.close()
